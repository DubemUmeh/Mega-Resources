        ...
        <section className="rounded-[1.5rem] border border-white/10 bg-[rgba(36,35,35,0.5)] p-6">
          ...
          <form action={saveBusinessSettings} className="mt-4 grid gap-3 md:grid-cols-2">
            {[
              ...
              <label key={n} className="text-sm text-muted-foreground">
                {l}
                <input
                  name={n}
                  defaultValue={v || ""}
            ...
            <button className="rounded-full bg-blue-600 px-5 py-2 text-sm font-semibold text-white md:col-span-2">
              Save profile
            </button>
          </form>
        </section>
        ...
        <section className="rounded-[1.5rem] border border-white/10 bg-[rgba(36,35,35,0.5)] p-6">
          ...
          <div className="mt-3 grid gap-2 text-sm text-muted-foreground">
            <p>
              ...
              <span className="text-foreground">{connection?.googleEmail || "Not connected"}</span>
            </p>
            <p>
              Gmail API status:{" "}
              <span className={connection && !connection.revokedAt ? "text-green-300" : "text-red-300"}>
                    ...
                    <td className="flex gap-2 py-2">
                      {session.role === "SUPER_ADMIN" && (
                        <>
                          ...
                          >
                            <button className="text-amber-300">Disable</button>
                          </form>
                          <form
                            action={async () => {
                              "use server";
                          ...
                          >
                            <button className="text-red-300">Remove</button>
                          </form>
                        </>
                      )}
                    </td>
